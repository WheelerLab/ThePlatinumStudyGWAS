...
#topsnp <- data.frame(SNP=geno['rs62283056',]) %>% mutate(FID=rownames(topsnp))
...
#data <- inner_join(topsnp,pheno,by="FID")

#write.table(data, pheno.dir %&% "rs62283056_PtStudy.pheno.cov.txt",row.names=F,quote=F)

data<-read.table(pheno.dir %&% "rs62283056_PtStudy.pheno.cov.txt",header=TRUE)
data <- mutate(data,dosegroup=ifelse(CumlCispdose<=300,0,1)) 
demo <- read.sas7bdat("/Volumes/dolan-lab/hwheeler/ThePlatinumStudy/GWAS/Datasets_from_Ryan_Cook_20151106/demographics.sas7bdat") %>% mutate(IID=as.character(patno)) %>% dplyr::select(IID,C_HTCM) #more ht phenotypes in this dataset
hist(demo$C_HTCM)
data <- left_join(data,demo,by="IID")
newphenfile <- dplyr::select(data,-SNP)
write.table(newphenfile,pheno.dir %&% "PtStudy.phenofile",row.names=F,quote=F)
...
ggplot(databox,aes(x=SNPgt,y=-1*GM412, color=factor(dosegroup,labels=c("\u2264 300","> 300")))) + geom_boxplot()+ scale_color_discrete(name="Cisplatin dose") + xlab("rs62283056 genotype")+ylab("Hearing threshold (4-12 kHz)") +theme_bw()
